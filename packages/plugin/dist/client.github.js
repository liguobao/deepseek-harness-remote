  ...
  window.__ModuleLoader__.load({
    ...
    factory: (require2) => {
      ...
      function RemotePluginOptions(props) {
        let [open, setOpen] = React.useState(!1), [serverUrl, setServerUrl] = React.useState(""), [role, setRole] = React.useState("host"), [deviceName, setDeviceName] = React.useState(""), [hostEnrollment, setHostEnrollment] = React.useState("account"), [registrationCode, setRegistrationCode] = React.useState(""), [email, setEmail] = React.useState(""), [password, setPassword] = React.useState(""), [association, setAssociation] = React.useState(void 0), [loaded, setLoaded] = React.useState(!1), [writable, setWritable] = React.useState(!1), [busy, setBusy] = React.useState(!1), [notice, setNotice] = React.useState(void 0), [error, setError] = React.useState(void 0), applyView = (view) => {
          setServerUrl(view.config.serverUrl ?? "https://dsh.r2049.cn"), setRole(view.config.role === "client" ? "client" : "host"), setDeviceName(view.deviceName), setAssociation(view.association), setWritable(view.writable), setLoaded(!0);
        }, load = async () => {
        ...
        let save = async (event) => {
          if (event.preventDefault(), !!writable) {
            ...
            try {
              let result = await props.control("settings.configure", {
                serverUrl,
                role,
                ...role === "host" && hostEnrollment === "host_registration_code" ? { registrationCode } : { email, password }
              });
              applyView(result.settings), setNotice("Associated. Restart Harness to apply."), setRegistrationCode(""), setPassword("");
            } catch (reason) {
          ...
          let nextRole = role === "host" ? "client" : "host";
          if (setError(void 0), setNotice(void 0), association === void 0) {
            setRole(nextRole);
            return;
          }
          setBusy(!0);
          try {
            let view = await props.control("settings.role.set", { role: nextRole });
            applyView(view), setNotice("Mode changed. Restart Harness to apply.");
          } catch (reason) {
            setError(messageOf(reason));
          } finally {
            setBusy(!1);
          ...
          try {
            ...
            applyView(view), setEmail(""), setPassword(""), setRegistrationCode(""), setNotice("Signed out. Restart Harness to disconnect this mode.");
          } catch (reason) {
            setError(messageOf(reason));
          } finally {
            setBusy(!1);
          ...
          setOpen(!1);
        }, modeSwitch = React.createElement(
          "div",
          ...
          open ? React.createElement(
            ...
            loaded ? association !== void 0 ? React.createElement(
              ...
              React.createElement(
                ...
                React.createElement("span", null, association.account === void 0 ? "Authorization" : "Account"),
              ...
              React.createElement(
                ...
                notice === void 0 ? null : React.createElement("span", null, notice)
              ),
              ...
              role === "host" ? React.createElement(
                ...
                React.createElement(
                  "select",
                  {
                    ...
                    disabled: busy || !writable,
                    onChange: (event) => setHostEnrollment(event.target.value)
                  },
                  React.createElement("option", { value: "account" }, "Account password"),
                  React.createElement("option", { value: "host_registration_code" }, "Host registration code")
                )
              ) : null,
              role === "host" && hostEnrollment === "host_registration_code" ? React.createElement(
                "label",
                null,
        ...
        return supported ? React.createElement(
          ...
          open ? React.createElement(
            ...
            React.createElement(
              ...
              React.createElement("div", { className: "dshRemoteDevices" }, devices.length === 0 ? React.createElement("p", null, "No authorized remote Host for this account.") : devices.map((device) => React.createElement("button", {
                type: "button",
                key: device.deviceId,
              ...
              status?.hostAuthorizationAvailable && status.host !== void 0 ? React.createElement(
                ...
                status.host.accountRequired ? React.createElement(
                  ...
                  }, busy ? "Registering\u2026" : "Use registration code")
                ) : null
              ) : null,
              error === void 0 ? null : React.createElement("p", { className: "dshRemoteError", role: "alert" }, error)
            )
