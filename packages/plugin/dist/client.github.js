  ...
  window.__ModuleLoader__.load({
    ...
    factory: (require2) => {
      ...
      function RemotePluginOptions(props) {
        let { t } = props, [open, setOpen] = React.useState(!1), [serverUrl, setServerUrl] = React.useState(""), [role, setRole] = React.useState("host"), [deviceName, setDeviceName] = React.useState(""), [hostEnrollment, setHostEnrollment] = React.useState("account"), [registrationCode, setRegistrationCode] = React.useState(""), [email, setEmail] = React.useState(""), [password, setPassword] = React.useState(""), [associations, setAssociations] = React.useState({}), [loaded, setLoaded] = React.useState(!1), [writable, setWritable] = React.useState(!1), [busy, setBusy] = React.useState(!1), [notice, setNotice] = React.useState(void 0), [error, setError] = React.useState(void 0), [settingsView, setSettingsView] = React.useState(void 0), persistedRole = settingsView?.config.role === "client" ? "client" : "host", persistedServerUrl = settingsView?.config.serverUrl ?? "https://dsh.r2049.cn", association = associations[role], roleDirty = settingsView !== void 0 && role !== persistedRole, serverDirty = settingsView !== void 0 && serverUrl !== persistedServerUrl, authorizationDirty = email !== "" || password !== "" || registrationCode !== "", draftDirty = roleDirty || serverDirty || authorizationDirty, applyView = (view) => {
          let viewRole = view.config.role === "client" ? "client" : "host";
          setSettingsView(view), setServerUrl(view.config.serverUrl ?? "https://dsh.r2049.cn"), setRole(viewRole), setDeviceName(view.deviceName), setAssociations(view.associations ?? (view.association === void 0 ? {} : { [viewRole]: view.association })), setWritable(view.writable), setLoaded(!0);
        }, load = async () => {
        ...
        let save = async (event) => {
          if (event?.preventDefault(), !!writable) {
            ...
            try {
              ...
              if (roleDirty && !serverDirty && association !== void 0) {
                let view = await props.control("settings.role.set", { role });
                applyView(view), setNotice({ key: "modeSavedReused" });
                return;
              }
          ...
          nextRole !== role && (setError(void 0), setNotice(void 0), setRole(nextRole));
        }, logout = async () => {
          ...
          React.createElement(
            "div",
            { className: "dshRemoteRoleSwitch", role: "group", "aria-label": t("pluginMode") },
          ...
          open ? React.createElement(
            ...
            loaded ? association !== void 0 ? React.createElement(
              ...
              role === "host" && hostEnrollment === "host_registration_code" ? React.createElement(
                ...
                {
                  className: "dshRemoteAuthPanel",
                  role: role === "host" ? "tabpanel" : void 0,
                  id: role === "host" ? "dsh-remote-auth-account-panel" : void 0,
                  "aria-labelledby": role === "host" ? "dsh-remote-auth-account-tab" : void 0
                },
                React.createElement(
                  "div",
                  { className: "dshRemoteField" },
      ...
      function apply(ctx) {
        ...
        ctx.effect(() => ctx.locale.register(localeNamespace, { zh, en }), "dsh-remote: dictionaries"), ctx.effect(installStyle, "dsh-remote: client styles"), ctx.slots.inject("sidebar.footer.action", () => ctx.slots.register({
