...
如需固定 GitHub Release，也可以安装 `github:liguobao/deepseek-harness-remote#v0.3.25`。

安装后请重启 Harness。

`0.3.25` Client 继续兼容 `0.3.15` Host 的远端 Workspace 与会话；远端命令目录、文件查看、图片分块传输等后续能力只会在所选 Host 支持时启用。
